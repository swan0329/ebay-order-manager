...
describe("pricing recommendation approval boundary", () => {
  beforeEach(() => {
    ...
    mocks.transaction.mockImplementation(async (callback) => callback({
      ...
      listingPriceApproval: { create: mocks.priceApprovalCreate },
    }));
  });
  ...
  it("copies an approved snapshot only after an explicit apply action", async () => {
    ...
    expect(mocks.productUpdate).toHaveBeenCalledWith(expect.objectContaining({
      where: { id: "product-1" },
      data: expect.objectContaining({ finalListingPriceUsd: "15.90" }),
    }));
    expect(mocks.priceApprovalCreate).toHaveBeenCalled();
  });
});
