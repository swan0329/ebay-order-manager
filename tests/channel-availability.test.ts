...
describe("채널 판매 가능 수량", () => {
  ...
  it("포카마켓 빠른구매 재고 전체를 판매 가능 수량에 반영한다", () => {
    expect(resolveChannelAvailability({ ...base, isSoldOut: false, pocamarketAvailableCount: 7 })).toMatchObject({ availabilityStatus: "AVAILABLE", quantity: 7, pocamarketListingQuantity: 7 });
    expect(resolveChannelAvailability({ ...base, stockQuantity: 2, isSoldOut: false, pocamarketAvailableCount: 8 })).toMatchObject({ availabilityStatus: "AVAILABLE", quantity: 10, ownSellableQuantity: 2, pocamarketListingQuantity: 8 });
  });

  it("내부 품절 표시는 포카마켓 조달 가능 상품을 판매중지시키지 않는다", () => {
    expect(resolveChannelAvailability({ ...base, status: "sold_out", isSoldOut: false, pocamarketAvailableCount: 2 })).toMatchObject({ availabilityStatus: "AVAILABLE", quantity: 2, actionable: true });
  });
  ...
  it("수집값이 아예 없을 때만 전송하지 않고, 오래된 마지막 수집값은 사용한다", () => {
    ...
    expect(resolveChannelAvailability({ ...base, isSoldOut: false, pocamarketAvailableCount: 2, pocamarketSyncedAt: new Date(0) })).toMatchObject({ availabilityStatus: "AVAILABLE", quantity: 2, actionable: true });
  });
