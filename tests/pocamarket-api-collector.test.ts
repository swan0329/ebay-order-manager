...
describe("Pocamarket total request deadline", () => {
  it("stops retry backoff at the deadline instead of starting another request", async () => {
    const clock = vi.spyOn(Date, "now").mockReturnValue(1000);
    const request = vi.fn().mockRejectedValue(new Error("network"));
    const wait = vi.fn(async (milliseconds: number) => {
      clock.mockReturnValue(1000 + milliseconds);
    });
    try {
      await expect(fetchPocamarketProductState("1", config, {
        fetch: request, sleep: wait, deadlineAt: 1050,
  ...
  it("keeps rate limiting a safety stop when the deadline ends during backoff", async () => {
    const clock = vi.spyOn(Date, "now").mockReturnValue(1000);
    const request = vi.fn().mockResolvedValue(new Response(null, { status: 429 }));
    try {
      await expect(fetchPocamarketProductState("1", config, {
        fetch: request,
        sleep: async () => { clock.mockReturnValue(1050); },
        deadlineAt: 1050,
      })).rejects.toBeInstanceOf(PocamarketBlockingError);
      expect(request).toHaveBeenCalledTimes(1);
    } finally { clock.mockRestore(); }
