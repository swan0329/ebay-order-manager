...
vi.mock("@/lib/prisma", () => ({ prisma: { listingDraft: { findMany: findDrafts }, product: { findMany: findProducts }, pricingSettings: { findUnique: vi.fn().mockResolvedValue(null) } } }));
...
describe("eBay operations classification", () => {
  beforeEach(() => { vi.clearAllMocks(); findDrafts.mockResolvedValue([]); findProducts.mockResolvedValue([]); });
  ...
  it("keeps only the newest draft per product", async () => {
    findDrafts.mockResolvedValue([
      { id:"new",sourceInventoryId:"p",sku:"A",title:"new",price:10,quantity:1,status:"draft",errorSummary:null,updatedAt:new Date() },
      { id:"old",sourceInventoryId:"p",sku:"A",title:"old",price:9,quantity:1,status:"failed",errorSummary:"old",updatedAt:new Date(0) },
    ]);
    planInventory.mockResolvedValue({ missingPrice: [], rows: [] });
    const result = await getEbayOperations("user");
