...
import { isDeductibleOrder, statusAfterStockChange } from "@/lib/inventory";
...
describe("isDeductibleOrder", () => {
  it("rejects cancelled orders from either sales channel", () => {
    expect(
      isDeductibleOrder({ orderStatus: "CANCELLED", fulfillmentStatus: "NOT_STARTED" }),
    ).toBe(false);
  });

  it("still deducts fulfilled orders when the item was never deducted", () => {
    expect(
      isDeductibleOrder({ orderStatus: "OPEN", fulfillmentStatus: "FULFILLED" }),
    ).toBe(true);
  });
});
