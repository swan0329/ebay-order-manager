...
판매가는 두 파일 모두 `src/lib/listing-price.ts`가 정한다. 사람이 상품별 eBay 판매가(`ebay_price`, USD)를 입력했다면 그 확정값을 우선하며, 입력하지 않은 상품만 포카마켓 가격(`sale_price`, KRW)과 마진 공식으로 계산한다. 두 값이 모두 없는 상품은 파일에서 제외하며, 남는 대상이 없으면 파일 대신 `422`와 안내 문구를 반환한다.
...
eBay `IMAGE_REPAIR` 실행은 `product_upload_jobs`에 묶음별로 영구 저장한다. 응답 후 서버 작업으로 계속 처리하며 전역 메뉴가 상태를 복원하고 중단된 대기 작업을 다시 깨운다. 성공은 현재 워터마크·옵션 이미지 해시와 함께 기록되어 같은 상태의 항목은 처리 후보에서 제외한다.
