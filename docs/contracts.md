...
신규등록용 `/api/export/ebay-listings`와 `/api/listing-upload/inventory/export`는 최근 전체 활성상품 보고서가 있어야 한다. 대상은 공급 가능, 이미지 완료, 판매가 확정 가능, eBay 비활성·미연결 조건을 모두 만족해야 한다. 판매 수량은 내 판매 가능 재고와 포카마켓 빠른구매 구매 가능 수량의 합계다. Lens CSV는 같은 조건 중 Lens 승인 이미지를 쓰는 상품만 포함한다.

판매가는 두 파일 모두 `src/lib/listing-price.ts`가 정한다. 포카마켓 가격(`sale_price`, KRW)이 있으면 마진 계산가를 사용하고, 포카마켓 가격이 없는 상품만 사람이 입력한 eBay 판매가(`ebay_price`, USD)를 사용한다. 두 값이 모두 없는 상품은 파일에서 제외하며, 남는 대상이 없으면 파일 대신 `422`와 안내 문구를 반환한다.
...
`GET /api/ebay/operations`는 신규등록·가격/재고 변동·품절/판매중지 대상을 현재 내부 값과 마켓 재조회로 확인된 마지막 값으로 분류한다. 이미 목표 가격·수량과 실제 확인값이 같은 상품은 작업 대상 및 숫자에서 제외한다. `POST /api/ebay/operations`는 선택한 변동 또는 품절 항목을 먼저 `dryRun=true`로 보여 주며, 실제 전송은 `confirmed=true`가 필요하다. 외부 API가 쓰기 성공을 반환했더라도 재조회 값이 다르거나 확인할 수 없으면 완료로 기록하지 않는다.
...
Shopify 가격·재고 변동은 기존 상품의 옵션 가격과 재고 위치 수량만 수정하며 이미지 생성·교체·옵션 미디어 연결을 함께 실행하지 않는다. 품절 작업은 가격을 요구하거나 바꾸지 않고 해당 옵션들의 실제 재고 수량만 0으로 맞춘다. 두 작업은 Shopify 실제 옵션 가격과 재고 위치 수량을 재조회해 일치한 값만 `ProductListing` 기준값으로 저장한다. 화면의 `Shopify 실제상태 재확인`은 외부 쓰기 없이 같은 값을 읽어 이미 반영된 과거 작업의 내부 기준값과 완료 이력만 복구한다. Shopify 백그라운드 쓰기는 사용자별로 하나씩 직렬 처리하며, 2분 이상 중단된 이미지·가격·재고 작업만 멱등 실제상태 확인 경로로 재개한다. 신규등록은 중복 생성 위험 때문에 자동 재실행하지 않는다.
...
eBay `IMAGE_REPAIR` 실행은 `product_upload_jobs`에 묶음별로 영구 저장한다. 응답 후 서버 작업으로 계속 처리하며 전역 메뉴가 상태를 복원하고 중단된 대기 작업을 다시 깨운다. 성공은 eBay `GetItem` 재조회 대표사진과 제작 썸네일의 시각적 일치까지 확인한 뒤 현재 워터마크·옵션 이미지 해시와 함께 기록하며, 확인된 동일 상태의 항목만 처리 후보와 숫자에서 제외한다. 과거 쓰기 응답만 성공으로 저장된 작업은 재전송하지 않고 실제 대표사진을 재조회해 완료 또는 미확인으로 재분류한다.
...
기존 리스팅의 품절 판정은 내 판매 가능 재고(내 재고 - 미처리 주문 예약)와 마지막으로 수집한 포카마켓 빠른구매 재고를 함께 본다. 둘 다 0인 경우만 `품절`이며, 내 재고가 주문 예약 때문에 0이 된 경우는 `판매 보류`로 구분한다. 포카마켓 관측값이 한 번도 수집되지 않은 경우만 `포카마켓 재고 확인 필요`로 표시하고 자동 전송 대상에서 제외한다. 채널 수량은 내 판매 가능 재고와 포카마켓 빠른구매 구매 가능 수량의 합계다.

eBay 가격·재고 및 옵션 품절 전송은 승인된 상품 ID를 `product_upload_jobs`에 먼저 저장한 뒤 `LMS_REVISE_INVENTORY_STATUS` Sell Feed 백그라운드 작업으로 최대 2,000건을 한 파일에 제출한다. 단품은 Item ID로, 묶음 옵션은 Item ID와 SKU로 식별하고 상품 ID를 MessageID로 넣는다. eBay task ID를 모든 내부 작업에 보존하며 화면에는 대상·eBay 접수·결과 대기·성공·실패 수를 구분한다. `COMPLETED` 또는 `COMPLETED_WITH_ERROR` 뒤 내려받은 결과 파일의 CorrelationID별 Success/Warning만 성공으로 저장하고 오류 및 결과 누락은 실패로 남긴다. 중단된 과거 개별 전송은 2분 후 대량 파일 방식으로 전환하고, 전역 메뉴가 3초 간격으로 상태 확인을 재개한다. 완료 이력과 `ProductListing`은 100개 이하 트랜잭션 묶음으로 순차 저장해 운영 DB 연결 풀 경쟁을 피한다. 소량 자동화의 직접 호출만 요청당 4개·동시성 3 제한을 유지한다.

활성 eBay 대량 작업은 화면 상태 조회가 짧은 DB lease를 획득해 결과 확인을 계속 깨운다. 최초 요청이 끝난 뒤 30분 예약 작업까지 멈춰 있지 않으며, 같은 task ID의 파일 업로드·결과 다운로드·내부 완료 저장은 lease 소유 요청 하나만 수행한다.
