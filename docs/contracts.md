...
eBay 가격·재고 및 옵션 품절 전송은 승인된 상품 ID를 `product_upload_jobs`에 먼저 저장한 뒤 백그라운드에서 처리한다. `ReviseInventoryStatus`는 공식 규격대로 요청당 최대 4개 옵션, 동시 최대 3개 요청으로 제한한다. 단품은 Item ID로, 묶음 옵션은 Item ID와 SKU로 전송·검증 대상을 식별한다. 전송 후 같은 Item ID의 옵션을 묶어 `GetItem`으로 최대 2번 재조회하며 실제 가격과 판매 가능 수량이 일치한 항목만 성공으로 저장한다. 24개 상품마다 결과와 진행률을 저장하고 같은 사용자의 대량작업은 하나만 실행한다. 서버 작업이 중단되면 5분 뒤 대기로 되돌리고, 전역 메뉴의 30초 재개 신호 또는 30분 주문 수집에서 이어서 처리한다. 완료된 내부 이력 기록은 단일 DB 트랜잭션으로 저장해 연결 풀 경쟁을 만들지 않는다.
