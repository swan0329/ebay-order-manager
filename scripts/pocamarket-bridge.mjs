...
function adbRun(...args) {
  return execFileSync(adb, args, { encoding: "utf8", timeout: 20000, windowsHide: true }).trim();
}

function adbBuffer(...args) {
  return execFileSync(adb, args, { encoding: "buffer", timeout: 20000, windowsHide: true });
}
...
async function purchaseOne(serial, job, purchasedQuantity) {
  ...
  // 완료 수량은 주문 화면의 명시적인 1장 결제 완료 확인만 증가시킨다.
