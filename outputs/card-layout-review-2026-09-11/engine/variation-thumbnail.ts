...
import { createPreparedListingImage } from "./ebay-watermarked-images";
...
export const variationThumbnailRenderVersion = "v10-portrait-rounded-all-cards";
...
export async function createVariationThumbnailBase(input: VariationThumbnailInput) {
  ...
  // against the actual 1000px group canvas, not a tiny member-card size.
  return base
...
async function watermarkLayer(input: VariationThumbnailInput) {
  ...
  const placements = await Promise.all(variationWatermarkPlacements({
    canvasWidth: SIZE, canvasHeight: SIZE, headerHeight: HEADER_HEIGHT,
