...
export function EbayOperationsClient({ initial, initialChannel = "EBAY" }: { initial: OperationsClientData; initialChannel?: Channel }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      if (job.active && Date.now() - lastInventoryKickAt.current >= 30_000) {
        lastInventoryKickAt.current = Date.now();
        void fetch("/api/ebay/operations/inventory-jobs", { method: "POST" });
      }
      if (!job.active) {
  ...
  const inventoryTiming = useMemo(() => {
    ...
    const eta = job.completed > 0 && job.completed < job.total ? Math.ceil((elapsedSeconds / job.completed) * (job.total - job.completed)) : null;
    return { elapsed: elapsedSeconds, eta };
  ...
  return <div className="mt-6 space-y-4">
    ...
    {channel === "EBAY" && data.inventoryJob && data.inventoryJob.total > 0 && <div className="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950"><div className="flex flex-wrap items-center justify-between gap-2"><b>eBay 가격·재고 서버 작업 {data.inventoryJob.active ? "진행 중" : "완료"}</b><span>완료 {data.inventoryJob.completed}/{data.inventoryJob.total} · 성공 {data.inventoryJob.succeeded} · 실패·미확인 {data.inventoryJob.failed} · 처리 중 {data.inventoryJob.running} · 대기 {data.inventoryJob.pending}</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-sky-100"><div className="h-full rounded-full bg-sky-600 transition-[width]" style={{width:`${data.inventoryJob.total ? Math.round((data.inventoryJob.completed / data.inventoryJob.total) * 100) : 0}%`}}/></div><p className="mt-2 text-xs">{data.inventoryJob.active ? `${inventoryTiming.elapsed}초 경과 · ${inventoryTiming.eta == null ? "첫 처리 묶음의 eBay 재조회 중" : `현재 속도 기준 약 ${inventoryTiming.eta}초 남음`}. ` : ""}24개 상품 묶음마다 결과를 저장하고, eBay 실제 가격·판매 가능 수량을 다시 확인한 항목만 성공 처리합니다. 메뉴를 이동하거나 창을 닫아도 계속됩니다.</p>{!data.inventoryJob.active && data.inventoryJob.failed > 0 && <p className="mt-2 font-semibold text-amber-900">실패·미확인 항목은 아래 결과에 이유가 남아 있으며, ‘실패·미확인 재시작’으로 해당 항목만 다시 처리할 수 있습니다.</p>}</div>}
    ...
    <section className="overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-sm">
      ...
      <div className="grid gap-3 p-4 lg:grid-cols-[1.3fr_1fr]">
        <div className="rounded-xl border bg-zinc-50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-zinc-500">작업 상태</p><p className="mt-1 font-bold">{busy ? phase === "sending" ? `${channel === "EBAY" ? "eBay" : "Shopify"} 작업 등록 중` : "대상 자동 검증 중" : serverJobActive ? "eBay 서버 작업 진행 중 — 메뉴 이동 가능" : preview ? "자동 검증 제외 항목 확인" : result ? result.failed ? "일부 실패 — 해당 항목만 재시작 가능" : "작업 완료" : "대기"}</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${busy || serverJobActive ? "bg-blue-100 text-blue-800" : result?.failed ? "bg-amber-100 text-amber-800" : result ? "bg-emerald-100 text-emerald-800" : "bg-zinc-200 text-zinc-700"}`}>{busy ? `${elapsed}초` : serverJobActive ? `${data.inventoryJob?.completed ?? 0}/${data.inventoryJob?.total ?? 0}` : result ? `성공 ${result.succeeded} · 실패 ${result.failed}` : `선택 ${selected.length}건`}</span></div>
          ...
          <p className="mt-3 text-sm text-zinc-600">{busy ? phase === "sending" ? `선택 ${selected.length}건을 서버 작업으로 안전하게 등록 중입니다.` : `현재 대상 검증 중 · ${elapsed}초 경과 · ${estimatedText}` : serverJobActive ? `${inventoryTiming.elapsed}초 경과 · ${inventoryTiming.eta == null ? "첫 묶음 확인 중" : `약 ${inventoryTiming.eta}초 남음`} · 완료 수는 eBay 재조회까지 끝난 항목만 올라갑니다.` : preview ? `${preview.rows.length}개 항목은 자동 검증에서 제외되어 마켓에 전송하지 않았습니다.` : result ? message : tab === "imageRepair" ? channel === "EBAY" ? `옵션 구성과 옵션별 사진은 유지하고 묶음 대표사진만 교체합니다. ${estimatedText}` : `묶음은 제작 썸네일을 첫 사진으로, 옵션은 개별 카드 사진으로 연결합니다. ${estimatedText}` : "목록에서 항목을 선택한 뒤 ‘작업 시작’을 누르면 자동 검증을 통과한 항목만 바로 적용합니다."}</p>
        </div>
