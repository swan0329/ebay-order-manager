...
type ReviseRow = { productId: string; sku: string; productName: string; itemId: string; quantity: number; price: number | null; previousQuantity: number | null; previousPrice: number | null };
type UnavailableRow = ReviseRow & { reason: "품절" | "판매중지" };
type Data = { create: CreateRow[]; change: ReviseRow[]; unavailable: UnavailableRow[]; limits: { createBatch: number; reviseBatch: number }; summary?: { createReady: number; createNeedsReview: number; createCountMeaning: string } };
type PreviewRow = { id?: string; productId?: string | null; sku: string; title?: string; name?: string; productName?: string; itemId?: string; price: number | null; previousPrice?: number | null; quantity: number | null; previousQuantity?: number | null; imageCount?: number; valid?: boolean; issues?: Array<{ field: string; message: string }> };
...
export function EbayOperationsClient({ initial }: { initial: Data }) {
  ...
  return <div className="mt-6 space-y-4">
    ...
    {channel === "EBAY" && data.summary && <div className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-950"><b>신규등록 숫자의 의미:</b> {data.summary.createCountMeaning}. 검증 전·실패 초안 {data.summary.createNeedsReview.toLocaleString()}건은 신규등록 수에 포함하지 않습니다.</div>}
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      <div className="max-h-[620px] overflow-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="sticky top-0 bg-zinc-100"><tr><th className="p-3">선택</th><th>마켓</th><th>작업</th><th>SKU</th><th>상품명</th><th>마켓 상품 ID</th><th>기존 가격 → 전송 가격</th><th>기존 수량 → 전송 수량</th><th>판정</th></tr></thead><tbody>
        {rows.map((row) => { const create = tab === "create" ? row as CreateRow : null; const revise = tab !== "create" ? row as ReviseRow : null; const id = create?.productId ?? revise!.productId; return <tr key={id} className="border-t"><td className="p-3"><input aria-label={`${row.sku} 선택`} type="checkbox" checked={selected.includes(id)} onChange={() => toggle(id)} /></td><td>{channel === "EBAY" ? "eBay" : "Shopify"}</td><td>{actionName(tab)}</td><td>{row.sku}</td><td className="max-w-sm truncate">{create?.name ?? revise?.productName}</td><td>{revise?.itemId ?? "신규"}</td><td>{revise ? `${money(revise.previousPrice)} → ${money(revise.price)}` : money(create?.price)}</td><td>{revise ? `${revise.previousQuantity ?? "-"} → ${revise.quantity}` : create?.quantity ?? "-"}</td><td>{tab === "unavailable" ? (row as UnavailableRow).reason : create?.error ?? create?.status ?? "변경 필요"}</td></tr>; })}
        {!rows.length && <tr><td colSpan={9} className="p-10 text-center text-zinc-500">현재 이 마켓에서 실제 처리할 항목이 없습니다.</td></tr>}
      </tbody></table></div>
    </section>
    {preview && <section className={`rounded-2xl border p-5 ${preview.valid ? "border-violet-300 bg-violet-50" : "border-red-300 bg-red-50"}`}><div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="text-lg font-bold">전송 전 최종 확인 · {preview.rows.length}건</h2><p className="mt-1 text-sm">아래 가격·수량·이미지·검증 결과가 실제 전송 내용입니다. 예상 {Math.ceil(estimate.minimum / 60)}~{Math.max(1, Math.ceil(estimate.maximum / 60))}분(마켓 응답에 따라 변동)</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${preview.valid ? "bg-emerald-100 text-emerald-800" : "bg-red-100 text-red-800"}`}>{preview.valid ? "전송 가능" : "전송 불가"}</span></div><div className="mt-4 max-h-80 overflow-auto rounded-xl border bg-white"><table className="w-full min-w-[850px] text-left text-sm"><thead className="sticky top-0 bg-zinc-100"><tr><th className="p-3">SKU</th><th>상품명</th><th>가격</th><th>수량</th><th>이미지</th><th>검증 결과</th></tr></thead><tbody>{preview.rows.map((row) => <tr key={row.id ?? row.productId ?? row.sku} className="border-t"><td className="p-3 font-medium">{row.sku}</td><td>{row.title ?? row.name ?? row.productName ?? "-"}</td><td>{row.previousPrice !== undefined ? `${money(row.previousPrice)} → ${money(row.price)}` : money(row.price)}</td><td>{row.previousQuantity !== undefined ? `${row.previousQuantity ?? "-"} → ${row.quantity ?? "-"}` : row.quantity ?? "-"}</td><td>{row.imageCount == null ? "-" : `${row.imageCount}장`}</td><td className={row.valid === false ? "text-red-700" : "text-emerald-700"}>{row.issues?.length ? row.issues.map((issue) => issue.message).join(" / ") : "통과"}</td></tr>)}</tbody></table></div><div className="mt-4 flex justify-end gap-2"><button onClick={() => setPreview(null)} disabled={busy} className="rounded-lg border bg-white px-4 py-2">취소·목록으로</button><button onClick={() => void run()} disabled={!preview.token || !preview.valid || busy} className="rounded-lg bg-rose-700 px-4 py-2 font-bold text-white disabled:opacity-40">위 내용을 확인했고 실제 전송</button></div></section>}
