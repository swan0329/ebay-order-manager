...
const duration = (seconds: number) => seconds < 60 ? `${seconds}초` : `${Math.floor(seconds / 60)}분 ${seconds % 60}초`;
...
export function EbayOperationsClient({ initial, initialChannel = "EBAY" }: { initial: OperationsClientData; initialChannel?: Channel }) {
  ...
  const estimatedText = channel === "EBAY" && (tab === "change" || tab === "unavailable")
    ? "접수 준비 5초~1분 · eBay 처리 1~10분 예상"
    : `${duration(estimatedSeconds.minimum)}~${duration(estimatedSeconds.maximum)} 예상`;
  ...
  useEffect(() => {
    if (!data.inventoryJob?.active && !data.shopifyJob?.active) return;
    const timer = window.setInterval(() => setJobClock(Date.now()), 1_000);
    return () => window.clearInterval(timer);
  }, [data.inventoryJob?.active, data.shopifyJob?.active]);
  ...
  async function refresh() {
    ...
    try {
      ...
      } else if (channel === "SHOPIFY" && ids.length > 0 && (tab === "change" || tab === "unavailable")) {
        setPhase("preview");
        setMessage("Shopify 실제 가격·재고를 읽기 전용으로 확인하고 있습니다.");
        const response = await fetch("/api/ebay/operations", {
          method: "POST", headers: { "content-type": "application/json" },
          body: JSON.stringify({ action: tab === "change" ? "CHANGE" : "UNAVAILABLE", productIds: ids, dryRun: true, channel: "SHOPIFY", reconcileChannelState: true }),
        });
        const body = await response.json();
        if (!response.ok) throw new Error(body.error ?? "Shopify 가격·재고 재확인에 실패했습니다.");
        setData({ ...body.operations, shopifyJob: body.job });
        const unresolved = (body.outcomes ?? []).filter((outcome: { current: boolean }) => !outcome.current);
        setMessage(unresolved.length
          ? `Shopify 실제 재확인 완료: ${body.completed ?? 0}건 완료 · ${unresolved.length}건 실제값 불일치`
          : `Shopify 실제 재확인 완료: ${body.completed ?? 0}건 완료 · 현재 미완료 0건`);
        setResult({ succeeded: body.completed ?? 0, failed: unresolved.length, rows: (body.outcomes ?? []).map((outcome: { sku: string; productId: string; current: boolean; reason: string }) => ({ sku: outcome.sku, productId: outcome.productId, action: tab, status: outcome.current ? "성공" : "실패", message: outcome.reason })) });
      } else {
  ...
  async function send(previewData: Preview, targetIds: string[], actionTab: Tab) {
    ...
    try {
      ...
      if (output.queued) {
        ...
        // 등록 직후 목록 재확인은 백그라운드 작업과 경쟁하고, Shopify에서는 아직
        // 반영 전 값을 실패처럼 보여 줄 수 있다. 상태 폴링이 완료 응답을 받은 뒤
        // 최신 작업 목록까지 함께 갱신하므로 여기서는 작업번호만 즉시 표시한다.
        setPreview(null); setSelected([]); setResult(null);
        return;
      }
  ...
  return <div className="mt-6 space-y-4">
    ...
    {channel === "SHOPIFY" && <ShopifyVariantCardTheme />}
