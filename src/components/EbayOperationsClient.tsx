...
export function EbayOperationsClient({ initial, initialChannel = "EBAY" }: { initial: OperationsClientData; initialChannel?: Channel }) {
  ...
  async function startAutomatically(targetIds = selected) {
    ...
    const controller = new AbortController(); abortRef.current = controller;
    try {
      const response = await fetch("/api/ebay/operations", { method: "POST", headers: { "content-type": "application/json" }, signal: controller.signal, body: JSON.stringify({ action: tab === "create" ? "CREATE" : tab === "change" ? "CHANGE" : tab === "unavailable" ? "UNAVAILABLE" : tab === "imageRepair" ? "IMAGE_REPAIR" : "REVIEW", productIds: targetIds, dryRun: true, channel }) });
      const body = await response.json();
  ...
  return <div className="mt-6 space-y-4">
    ...
    <section className="overflow-hidden rounded-2xl border border-zinc-300 bg-white shadow-sm">
      ...
      <div className="grid gap-3 p-4 lg:grid-cols-[1.3fr_1fr]">
        <div className="rounded-xl border bg-zinc-50 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-semibold text-zinc-500">작업 상태</p><p className="mt-1 font-bold">{busy ? phase === "sending" ? `${channel === "EBAY" ? "eBay" : "Shopify"} 전송 진행 중` : "대상 검증·미리보기 중" : preview ? "사람의 최종 확인 대기" : result ? result.failed ? "일부 실패 — 재시작 가능" : "작업 완료" : "대기"}</p></div><span className={`rounded-full px-3 py-1 text-sm font-bold ${busy ? "bg-blue-100 text-blue-800" : result?.failed ? "bg-amber-100 text-amber-800" : result ? "bg-emerald-100 text-emerald-800" : "bg-zinc-200 text-zinc-700"}`}>{busy ? `${elapsed}초` : result ? `성공 ${result.succeeded} · 실패 ${result.failed}` : `선택 ${selected.length}건`}</span></div>
          ...
          <p className="mt-3 text-sm text-zinc-600">{busy ? phase === "sending" ? `선택 ${selected.length}건 처리 중 · ${elapsed}초 경과 · ${estimatedText}. Shopify 이미지 생성·순서 변경·옵션 연결 확인을 차례로 수행합니다.` : `현재 대상 검증 중 · ${elapsed}초 경과 · ${estimatedText}` : preview ? `${preview.rows.length}개 항목은 자동 검증에서 제외되어 마켓에 전송하지 않았습니다.` : result ? message : tab === "imageRepair" ? `묶음은 제작 썸네일을 첫 사진으로, 옵션은 개별 카드 사진으로 연결합니다. ${estimatedText}` : "목록에서 항목을 선택한 뒤 ‘작업 시작’을 누르면 자동 검증을 통과한 항목만 바로 적용합니다."}</p>
        </div>
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      {busy && <div className="border-b bg-zinc-50 p-4"><div className="mb-2 flex justify-between text-sm"><b>{phase === "sending" ? `${channel === "EBAY" ? "eBay" : "Shopify"} 전송 처리 중` : "데이터 확인 중"}</b><span>{elapsed}초 경과 · {estimatedText}</span></div><div className="h-2 overflow-hidden rounded bg-zinc-200"><div style={{ width: `${progress}%` }} className="h-full rounded bg-violet-600 transition-[width] duration-500" /></div>{phase === "sending" && <p className="mt-2 text-xs text-zinc-600">이미지 교체는 Shopify의 비동기 처리 완료와 실제 대표사진·옵션 연결 확인까지 기다립니다. 완료 확인 전에는 성공으로 표시하지 않습니다.</p>}</div>}
