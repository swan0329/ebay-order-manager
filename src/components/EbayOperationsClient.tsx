...
export function EbayOperationsClient({ initial, initialChannel = "EBAY" }: { initial: OperationsClientData; initialChannel?: Channel }) {
  ...
  return <div className="mt-6 space-y-4">
    ...
    <section className="overflow-hidden rounded-2xl border bg-white">
      ...
      {busy && <div className="border-b bg-zinc-50 p-4"><div className="mb-2 flex justify-between text-sm"><b>{phase === "sending" ? `${channel === "EBAY" ? "eBay" : "Shopify"} 전송 처리 중` : "데이터 확인 중"}</b><span>{duration(elapsed)} 경과 · {estimatedText}</span></div><div className="h-2 overflow-hidden rounded bg-zinc-200"><div style={{ width: `${progress}%` }} className="h-full rounded bg-violet-600 transition-[width] duration-500" /></div>{phase === "sending" && <p className="mt-2 text-xs text-zinc-600">{channel === "EBAY" ? "가격·재고 작업은 eBay 대량 파일로 접수한 뒤 결과 파일에서 성공·실패를 확정합니다." : "이미지 교체는 Shopify의 비동기 처리 완료와 실제 대표사진·옵션 연결 확인까지 기다립니다. 완료 확인 전에는 성공으로 표시하지 않습니다."}</p>}</div>}
