...
// 한 번의 접수로 보낼 수 있는 최대 상품 수(등록 API 상한과 같다).
const maxPerRun = 500;
...
export function ChannelRegistrationControls() {
  ...
  async function startOne(kind: Channel, productIds: string[], expectedOptionProductIds?: string[]) {
    ...
    return publish(kind, { productIds, expectedOptionProductIds });
  }

  // 등록 가능한 상품을 한 번에 접수한다. 서버가 같은 조건으로 대상을 다시 고르므로
  // 화면이 들고 있던 목록이 오래됐어도 이미 등록된 상품을 다시 보내지 않는다.
  async function startAllOne(kind: Channel) {
    return publish(kind, { allEligible: true, limit: maxPerRun });
  }
  ...
  async function publish(
    ...
    const expectedOptionProductIds = payload.expectedOptionProductIds;
    const response = await fetch("/api/products/publish", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ channel: kind, ...payload, confirmed: true }),
    });
    ...
    try {
      const counts = await Promise.all(
        targets.map(async (kind) => {
          const response = await fetch(`/api/products/publish?channel=${kind}&limit=${maxPerRun}`, { cache: "no-store" });
          const body = (await response.json()) as {
            eligibleCount?: number;
            breakdown?: { readyCount: number; linkedExcludedCount: number; priceMissingCount: number };
            error?: string;
          };
          if (!response.ok) throw new Error(body.error ?? "등록 대상을 확인하지 못했습니다.");
          return { kind, eligible: body.eligibleCount ?? 0, breakdown: body.breakdown };
        }),
      );
      const summary = counts
        .map(({ kind, eligible, breakdown }) =>
          `${kind === "EBAY" ? "eBay" : "Shopify"}: 등록 가능 ${eligible.toLocaleString()}개` +
          (breakdown
            ? ` (미등록 후보 ${breakdown.readyCount.toLocaleString()}개 중 기존 연결 ${breakdown.linkedExcludedCount.toLocaleString()}개·가격 미입력 ${breakdown.priceMissingCount.toLocaleString()}개 제외)`
            : ""),
        )
        .join("\n");
      setMessage(summary);
      const total = counts.reduce((sum, item) => sum + item.eligible, 0);
      if (!total) return;
      const overflow = counts.some(({ eligible }) => eligible >= maxPerRun);
      if (
        !window.confirm(
          `${summary}\n\n등록 가능한 상품을 모두 실제 판매 등록합니다.` +
            `${overflow ? `\n한 번에 채널당 ${maxPerRun}개까지 접수되며 남은 상품은 다시 눌러 주세요.` : ""}` +
            `\n옵션 상품은 같은 묶음의 카드와 함께 등록되고 이미 등록된 상품은 건너뜁니다.\n\n계속할까요?`,
        )
      )
        return;
      const results = await Promise.allSettled(counts.filter(({ eligible }) => eligible > 0).map(({ kind }) => startAllOne(kind)));
      setMessage(
        results
          .map((result) =>
  ...
  return (
    ...
    <details className="group/registration">
      ...
      <p className="text-xs leading-relaxed text-zinc-500">{requestedCount === 1 ? "시험등록도 실제 판매 등록입니다. 아래에서 채널별 대상과 사진을 확인하세요." : "입력한 수만큼 미등록 후보를 선택합니다. 같은 묶음은 옵션으로 합쳐져 실제 판매상품 수와 다를 수 있습니다."} 옵션 우선 · 대표 썸네일 자동 생성 · 나머지는 단품 등록. 공급·이미지·가격 조건을 갖춘 상품을 한 번에 올리려면 아래 <b>등록 가능한 전체 등록</b>을 누르세요(채널당 최대 {maxPerRun.toLocaleString()}개씩 접수).</p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-zinc-100 pt-4">
        ...
        <span className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={() => void startAll()} disabled={starting || active} className="inline-flex h-10 items-center gap-2 rounded-md border border-emerald-700 px-4 text-sm font-bold text-emerald-800 hover:bg-emerald-50 disabled:border-zinc-200 disabled:text-zinc-400">
