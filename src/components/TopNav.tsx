...
export function TopNav({loginId}:{loginId:string}){
  ...
  const [ebayInventoryProgress,setEbayInventoryProgress]=useState<{active:number;submitted?:number;completed:number;total:number;succeeded:number;failed:number}|null>(null);
  ...
  return <>
    <aside data-app-sidebar className="fixed inset-y-0 left-0 z-50 hidden w-[272px] flex-col border-r border-zinc-200 bg-white md:flex">
      ...
      {ebayInventoryProgress&&ebayInventoryProgress.active>0&&<Link href="/ebay-operations" className="mx-3 mt-3 rounded-xl border border-sky-200 bg-sky-50 p-3 text-xs text-sky-950"><span className="flex items-center justify-between font-semibold"><span className="flex items-center gap-1.5"><ListChecks className="h-3.5 w-3.5"/>eBay 가격·재고 반영 중</span><span>접수 {ebayInventoryProgress.submitted??0}/{ebayInventoryProgress.total}</span></span><span className="mt-2 block h-1.5 overflow-hidden rounded-full bg-sky-100"><span className="block h-full bg-sky-600" style={{width:`${ebayInventoryProgress.total?ebayInventoryProgress.completed?Math.round(ebayInventoryProgress.completed/ebayInventoryProgress.total*100):(ebayInventoryProgress.submitted??0)>0?55:15:0}%`}}/></span></Link>}
