...
import {
  Camera, ChevronRight, Menu, Package, PackageOpen, PlugZap,
  Calculator, RefreshCw, Sparkles, Stamp, Truck, Users, X,
} from "lucide-react";
import { CatalogNavProgress, useCatalogNavProgress } from "@/components/CatalogNavProgress";
import { LogoutButton } from "@/components/LogoutButton";
import { productDataChangedEvent } from "@/lib/client-product-refresh";
...
const sections:NavSection[]=[
  {label:"업무 관리",items:[
    ...
    {href:"/products",label:"재고관리",icon:PackageOpen,matchPrefixes:["/products","/inventory","/inventory/movements"],excludePrefixes:["/inventory/photo-card-match","/products/ai-image-work","/products/watermark-settings"]},
  ...
  {label:"포토카드 이미지",items:[
    ...
    {href:"/products/watermark-settings",label:"워터마크 설정",icon:Stamp,matchPrefixes:["/products/watermark-settings"]},
...
export function TopNav({loginId}:{loginId:string}){
  const catalogProgress=useCatalogNavProgress();
  const pathname=usePathname();const [open,setOpen]=useState(false);
  const [counts,setCounts]=useState<NavCounts|null>(null);
  useEffect(()=>{let active=true;const load=()=>void fetch("/api/products/stats",{cache:"no-store"}).then(r=>r.ok?r.json():null).then(d=>{if(active&&d)setCounts({imagePendingCount:d.aiImagePendingCount??d.imagePendingCount??0,reviewCount:d.reviewCount??0})}).catch(()=>{});load();window.addEventListener(productDataChangedEvent,load);return()=>{active=false;window.removeEventListener(productDataChangedEvent,load)}},[pathname]);
  ...
  return <>
    <aside data-app-sidebar className="fixed inset-y-0 left-0 z-50 hidden w-[272px] flex-col border-r border-zinc-200 bg-white md:flex">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto"><CatalogNavProgress {...catalogProgress}/><Navigation pathname={pathname} counts={counts}/></div>
    ...
    {open&&<div className="fixed inset-0 top-14 z-40 overflow-y-auto bg-white md:hidden"><CatalogNavProgress {...catalogProgress}/><Navigation pathname={pathname} onNavigate={()=>setOpen(false)} counts={counts}/><div className="border-t p-4"><p className="mb-3 text-sm text-zinc-500">{loginId}</p><LogoutButton/></div></div>}
  </>;
}
