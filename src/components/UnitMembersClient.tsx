...
type EbayRepair = { itemId: string; sku: string; currentName: string; desiredName: string; productId: string; quantitySold: number };
...
export function UnitMembersClient({ items: initial }: { items: Item[] }) {
  ...
  async function applyRepairs() {
    const actionableRepairs = repairs.filter((row) => row.quantitySold === 0);
  ...
  return (
    <div className="space-y-3">
      ...
      <section className="rounded-xl border border-blue-200 bg-blue-50 p-4">
        ...
        {repairs.length ? <div className="mt-3 overflow-auto rounded-lg border bg-white"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-zinc-100"><tr><th className="p-2">Item ID</th><th>SKU</th><th>현재 옵션</th><th>수정 옵션</th><th>처리 가능</th></tr></thead><tbody>{repairs.map((row) => <tr key={`${row.itemId}:${row.sku}`} className="border-t"><td className="p-2">{row.itemId}</td><td>{row.sku}</td><td className="text-red-700">{row.currentName}</td><td className="font-semibold text-emerald-700">{row.desiredName}</td><td>{row.quantitySold ? <span className="text-amber-700">판매 {row.quantitySold}개 · 자동변경 불가</span> : <span className="text-emerald-700">가능</span>}</td></tr>)}</tbody></table><div className="border-t p-3 text-right"><button type="button" disabled={repairBusy || repairs.every((row) => row.quantitySold > 0)} onClick={() => void applyRepairs()} className="rounded-md bg-violet-600 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">처리 가능한 eBay 옵션 적용</button></div></div> : null}
      </section>
