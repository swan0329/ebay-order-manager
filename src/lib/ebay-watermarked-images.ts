...
// 등록 이미지 렌더가 실제로 읽는 설정만 캐시 키에 넣는다. 묶음 썸네일 전용
// 설정(variationWatermark*)까지 넣으면 그 값만 바꿔도 등록 이미지 전체가 새로
// 만들어지고 이전 세대가 R2에 그대로 쌓인다. 묶음 썸네일은 자기 키를 따로 쓴다.
export function listingImageRenderSignature(settings: ListingImageSettings) {
  return {
...
export function listingWatermarkKeys(
  imageUrl: string,
  logoIdentity: string,
  ...
  const digest = (payload: string) =>
    createHash("sha256")
      .update([listingWatermarkRenderVersion, imageUrl, logoIdentity, String(backgroundEligible), payload].join("\0"))
      .digest("hex");
  return {
    key: `products/channel-watermarked/${digest(JSON.stringify(listingImageRenderSignature(settings)))}.jpg`,
    // 예전 방식(설정 전체 해시)으로 이미 만들어 둔 같은 결과. 남아 있으면 그대로
    // 쓰고 다시 만들지 않는다. 판매채널에 보내는 주소도 그대로 유지된다.
    legacyKey: `products/channel-watermarked/${digest(JSON.stringify(settings))}.jpg`,
  };
}
...
export async function ensureListingWatermarkedImages(
  ...
  for (const imageUrl of normalizedUrls) {
    ...
    const { key, legacyKey } = listingWatermarkKeys(
      imageUrl,
      logoIdentity,
      settings,
      Boolean(options.backgroundEligible),
    );
    if (await getObjectFromR2(key)) {
      output.push(buildPublicR2Url(key));
      continue;
    }
    // 같은 결과를 예전 키로 이미 저장해 두었으면 다시 만들지 않는다.
    if (legacyKey !== key && (await getObjectFromR2(legacyKey))) {
      output.push(buildPublicR2Url(legacyKey));
      continue;
    }
