...
export async function refreshProcurementProduct<T extends Product>(product: T, userId?: string, force = false): Promise<T> {
  if (!force && !procurementRefreshDue(product)) {
    const [verified] = await withVerifiedProcurementEvidence([product]);
    if (verified === product) return product;
    force = true;
  }
  if (!/^\d+$/.test(product.pocamarketId ?? "")) return (await withVerifiedProcurementEvidence([product]))[0];
  const current = await prisma.product.findUniqueOrThrow({ where: { id: product.id } });
  if (!force && !procurementRefreshDue(current)) {
    const [verified] = await withVerifiedProcurementEvidence([current]);
    if (verified === current) return { ...product, ...current };
  }
  ...
  if (current.pocamarketLastAttemptAt && current.pocamarketLastAttemptAt > (current.pocamarketSyncedAt ?? new Date(0)) &&
    Date.now() - current.pocamarketLastAttemptAt.getTime() < 60_000) return (await withVerifiedProcurementEvidence([{ ...product, ...current }]))[0];
  ...
  if (!claimed.count) return (await withVerifiedProcurementEvidence([{ ...product, ...await prisma.product.findUniqueOrThrow({ where: { id: product.id } }) }]))[0];
  ...
  return (await withVerifiedProcurementEvidence([{ ...product, ...await prisma.product.findUniqueOrThrow({ where: { id: product.id } }) }]))[0];
}
