...
// API 키와 크레딧을 관리하는 Dewatermark 계정 화면. 결제 화면 주소가 바뀌면
// DEWATERMARK_BILLING_URL로 덮어쓸 수 있게 둔다.
const DEFAULT_BILLING_URL = "https://dewatermark.ai/ko/api-management";

export function getDewatermarkBillingUrl() {
  const configured = process.env.DEWATERMARK_BILLING_URL?.trim();
  if (!configured) return DEFAULT_BILLING_URL;
  // 잘못된 설정으로 화면에 위험한 주소가 노출되지 않도록 https만 허용한다.
  try {
