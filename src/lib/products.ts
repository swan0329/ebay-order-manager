import { memberFilter } from "@/lib/product-member";
...
export function productWhere(params: {
  ...
  relatedEbayItemIds?: string[];
}): Prisma.ProductWhereInput {
  const searchTerms = parseProductSearchTerms(params.q);
  ...
  } else if (params.upload === "review_required") {
    and.push({
  ...
  if (searchTerms.length) {
    and.push({
      OR: [...searchTerms.flatMap((term) => [
        ...
        { optionName: { contains: term, mode: "insensitive" as const } },
        { memo: { contains: term, mode: "insensitive" as const } },
        { ebayItemId: { contains: term, mode: "insensitive" as const } },
        { ebayTitle: { contains: term, mode: "insensitive" as const } },
      ]),
      ...(params.relatedProductIds?.length ? [{ id: { in: params.relatedProductIds } }] : []),
      ...(params.relatedEbayItemIds?.length ? [{ ebayItemId: { in: params.relatedEbayItemIds } }] : []),
      ],
    });
  }
