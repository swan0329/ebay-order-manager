...
export async function uploadShopifyProduct(productId:string,userId:string){
 ...
 await prisma.product.update({where:{id:productId},data:{shopifyProductId:result.productId,shopifyVariantId:result.variantId,shopifyInventoryItemId:result.inventoryItemId,shopifyStatus:result.status,shopifyLastUploadedAt:new Date(),shopifyUploadError:result.inventoryError??result.priceError??result.imageError}});
 await upsertProductListing({productId,channel:"SHOPIFY",externalId:result.productId,price:result.priceSynced?priceUsd:null,quantity:result.inventorySynced?availability.quantity:null,status:result.status,metadata:{variantId:result.variantId,inventoryItemId:result.inventoryItemId,source:"shopify_upload",inventorySynced:result.inventorySynced,inventoryError:result.inventoryError,priceSynced:result.priceSynced,priceError:result.priceError,imageSync:{status:result.imageError?"FAILED":"READY",sourceImageUrl:readyImage?.listingImageUrl??product.imageUrl,salesImageUrl:salesImage?.url??product.imageUrl,watermarkSignature:watermark.signature,watermarkApplied:salesImage?.applied??false},imageError:result.imageError}});
 if(result.inventoryError||result.priceError||result.imageError)throw new Error([result.inventoryError,result.priceError,result.imageError].filter(Boolean).join(" / "));
 return result;
}
