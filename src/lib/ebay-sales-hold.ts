...
type Variation = { SKU?: string; Quantity?: string; SellingStatus?: { QuantitySold?: string; ListingStatus?: string } };
...
export function observedEbayAvailable(item: Item, sku: string) {
  ...
  if (["Completed", "Ended"].includes(item.SellingStatus?.ListingStatus ?? "")) {
    return { available: 0, variation: rows.length > 0, ended: true };
  }
...
export function observedUnlinkedSingleAvailable(item: Item, itemId: string) {
  if (item.ItemID !== itemId || item.SKU?.trim() || item.Variations?.Variation) {
    throw new Error("상품번호 없는 단품만 미연결 판매 보류를 할 수 있습니다.");
  }
  return observedEbayAvailable({ ...item, SKU: "unlinked" }, "unlinked").available;
}

// Explicit administrator repair for legacy listings with no SKU/cost link.
// Zero only: never guess a product, change owned stock, or end the listing.
export async function ensureEbayUnlinkedSalesHold(userId: string, itemId: string) {
  const [product, group] = await Promise.all([
    prisma.product.findFirst({ where: { ebayItemId: itemId }, select: { id: true, sku: true } }),
    prisma.variationListingState.findFirst({ where: { userId, ebayItemId: itemId }, select: { id: true } }),
  ]);
  // Historical report links may refer to a card now sold in a new parent.
  // They do not make the abandoned single part of current price management.
  if (group) throw new Error("연결된 상품은 상품별 판매 보류를 사용해 주세요.");
  const replacement = product ? await prisma.variationListingState.findFirst({
    where: { userId, ebayItemId: { not: itemId }, includedProductIds: { array_contains: [product.id] } },
    select: { ebayItemId: true },
  }) : null;
  if (product && !replacement?.ebayItemId) throw new Error("연결된 상품은 상품별 판매 보류를 사용해 주세요.");
  ...
  const call = async (name: string, fields: string) => {
    ...
    if (!response.ok || !["Success", "Warning"].includes(data?.Ack)) throw new Error("eBay 미연결 판매 보류 요청을 확인하지 못했습니다.");
    return data;
  };
  const read = async () => observedUnlinkedSingleAvailable((await call("GetItem", `<ItemID>${xml(itemId)}</ItemID><DetailLevel>ReturnAll</DetailLevel>`)).Item, itemId);
  const before = await read();
  if (before === 0) return { itemId, available: 0, verified: true, changed: false };
  if (product && replacement?.ebayItemId) {
    // A saved group can supersede product.ebayItemId while its old single is
    // still active. Verify the exact replacement SKU before holding only the
    // stale single; do not rewrite the person's saved product connection.
    const item = (await call("GetItem", `<ItemID>${xml(replacement.ebayItemId)}</ItemID><DetailLevel>ReturnAll</DetailLevel>`)).Item as Item;
    if (item.ItemID !== replacement.ebayItemId || item.SellingStatus?.ListingStatus !== "Active") throw new Error("대체 묶음의 활성 상태를 확인하지 못했습니다.");
    const observed = observedEbayAvailable(item, product.sku);
    if (!observed.variation || observed.available <= 0) throw new Error("대체 묶음의 판매 가능 SKU를 확인하지 못했습니다.");
  }
  await ensureEbayOutOfStockControl(account);
  const reason = replacement ? "SUPERSEDED_SINGLE" : "UNLINKED_COST_UNKNOWN";
  const log = await prisma.syncLog.create({ data: { userId, type: "EBAY_SALES_HOLD", status: "PARTIAL", message: `${itemId}: 과거 단품 판매 보류`, rawJson: { itemId, before, reason, replacementItemId: replacement?.ebayItemId ?? null } } });
  await call("ReviseInventoryStatus", `<InventoryStatus><ItemID>${xml(itemId)}</ItemID><Quantity>0</Quantity></InventoryStatus>`);
  const after = await read();
  await prisma.syncLog.update({ where: { id: log.id }, data: { status: after === 0 ? "SUCCESS" : "FAILED", rawJson: { itemId, before, after, reason, replacementItemId: replacement?.ebayItemId ?? null } } });
  if (after !== 0) throw new Error("미연결 상품의 실제 판매 수량 0을 확인하지 못했습니다.");
  return { itemId, available: 0, verified: true, changed: true };
}
...
export async function ensureEbayProductSalesHold(userId: string, productId: string) {
  ...
  const before = await read();
  if (before.ended) {
    // GetItem's retained Quantity is historical after a listing ends. Do not
    // keep submitting a zero-quantity revision to a listing that cannot sell.
    await prisma.product.updateMany({ where: { id: product.id, ebayItemId: product.ebayItemId }, data: { listingStatus: "ENDED" } });
    await prisma.syncLog.create({ data: { userId, type: "EBAY_SALES_HOLD", status: "SUCCESS", message: `${product.sku}: eBay 실제 판매 종료 확인`, rawJson: { productId, itemId: product.ebayItemId, ended: true, available: 0 } } });
    return { sku: product.sku, available: 0, verified: true, changed: false, ended: true };
  }
