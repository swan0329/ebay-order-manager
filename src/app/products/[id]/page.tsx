...
export default async function ProductDetailPage({
  ...
  const imageSourceRows = await prisma.$queryRaw<
    Array<{
  ...
  const sourceImageUrl = imageSourceRows[0]?.sourceImageUrl?.trim() || null;
  const userImageRegistered = imageSourceRows[0]?.userImageRegistered ?? false;
  const effectiveImageSource =
    imageSource ?? (userImageRegistered ? "r2_user_uploaded" : null);
  const displayImage = {
    id: product.id,
    userImageRegistered,
    ...
    sourceImageUrl: sourceImageUrl ?? (product.ebayItemId
      ? product.ebayImageUrls[0] ?? `/api/products/${product.id}/ebay-display-image`
      : null),
  };
  const displayImageUrl = productDisplayImageUrl(displayImage);
        ...
        <div className="grid gap-4 lg:grid-cols-[340px_1fr]">
          <aside className="space-y-4">
            <section className="rounded-lg border border-zinc-200 bg-white p-4">
              <p className="mb-2 text-sm font-semibold text-zinc-700">대표 이미지 · {productDisplayImageLabel(displayImage)}</p>
              <div className="mb-4 flex aspect-square items-center justify-center overflow-hidden rounded-md bg-zinc-100">
                {displayImageUrl ? (
                  <img src={displayImageUrl} alt="" className="h-full w-full object-contain" />
                ) : (
