...
const schema = z.object({
  ...
  productIds: z.array(z.string().min(1)).min(2).max(250),
  watermarkText: z.string().trim().max(80).optional(),
  watermarkLogoDataUrl: z.string().max(3_000_000).optional(),
  watermarkOpacity: z.number().min(watermarkSettingLimits.opacity.min).max(watermarkSettingLimits.opacity.max).default(0.06),
  watermarkLogoSize: z.number().min(watermarkSettingLimits.logoSize.min).max(watermarkSettingLimits.logoSize.max).default(50),
  watermarkGap: z.number().min(watermarkSettingLimits.gap.min).max(watermarkSettingLimits.gap.max).default(25),
...
export async function POST(request: Request) {
  try {
    ...
    const logo = input.watermarkLogoDataUrl ? dataUrlBuffer(input.watermarkLogoDataUrl) : settings.logoUrl ? await downloadSavedLogo(settings.logoUrl) : null;
    const imageSourceRows = await prisma.$queryRaw<Array<{ id: string; imageSource: string | null }>>`
      SELECT "id", "image_source" AS "imageSource" FROM "products" WHERE "id" IN (${Prisma.join(input.productIds)})
    `;
    const imageSourceById = new Map(imageSourceRows.map((row) => [row.id, row.imageSource]));
    const buffer = await createVariationThumbnail({
      ...
      backgroundImageUrl: settings.backgroundUrl,
      backgroundEligible: input.productIds.map((id) => imageSourceById.get(id) === "lens_workbench"),
