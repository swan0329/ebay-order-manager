...
export async function POST(request: Request) {
  try {
    ...
    const products = (await withVariationListingMetadata(stored)).filter(hasListingPrice).map((product) => ({ ...product, imageUrl: imageById.get(product.id) ?? null, ebayImageUrls: [] }));
