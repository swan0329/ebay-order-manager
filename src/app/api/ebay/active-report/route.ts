...
export async function GET() {
  try {
    ...
    const [latest, reviewRequiredCount] = await Promise.all([
      ...
      prisma.product.count({
        where: { ebayItemId: { not: null }, listingStatus: "REVIEW_REQUIRED" },
      }),
    ]);
    return Response.json({ latest, reviewRequiredCount });
  } catch (error) {
