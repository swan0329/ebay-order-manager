...
async function main() {
  ...
  const r = await fetch(`https://${domain}/admin/api/${e.SHOPIFY_API_VERSION || '2025-10'}/graphql.json`, {method:'POST',headers:{'content-type':'application/json','X-Shopify-Access-Token':token},body:JSON.stringify({query:'query { products(first:250) { pageInfo { hasNextPage endCursor } nodes { id title status handle publishedAt variants(first:100) { nodes { id sku price image { url } } } media(first:100) { nodes { id status ... on MediaImage { image { url } } } } } } }'})});
