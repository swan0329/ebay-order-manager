...
async function main() {
  ...
  const r = await fetch(`https://${domain}/admin/api/${e.SHOPIFY_API_VERSION || '2025-10'}/graphql.json`, {method:'POST',headers:{'content-type':'application/json','X-Shopify-Access-Token':token},body:JSON.stringify({query:'query { currentAppInstallation { app { id title } accessScopes { handle } } shop { primaryDomain { url } } product(id: "gid://shopify/Product/15265031815536") { id title status handle onlineStoreUrl publishedAt onlineStorePreviewUrl } }'})});
