...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe'});try{
 ...
 const countsResponse=await countsReady;assert.equal(countsResponse.status(),200);const counts=await countsResponse.json();assert.equal(typeof counts.ebay.register,'number');
 await page.getByRole('heading',{name:'판매채널 자동 반영',exact:true}).waitFor({timeout:60000});
 await page.getByRole('link',{name:'내부 상품 추가',exact:true}).waitFor();
 const sales=page.locator('section').filter({has:page.getByRole('heading',{name:'판매채널 자동 반영',exact:true})}).first();
 await sales.scrollIntoViewIfNeeded();await sales.screenshot({path:'.codex-tmp/channel-ui-live-default.png'});
 ...
 await sales.screenshot({path:'.codex-tmp/channel-ui-live-expanded.png'});
