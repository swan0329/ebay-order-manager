...
export async function findProductImageCandidates(
  ...
  if (options.album) {
    extraClauses.push(
      Prisma.sql`REGEXP_REPLACE(LOWER(BTRIM(COALESCE("category", ''))), '[[:space:]_.-]+', '', 'g') = ${normalizeMatchMetadata(options.album)}`,
    );
  }
...
// 이미 연결된 eBay 사진과 그 상품의 내부 사진을 1:1로 감사할 때 사용한다.
// 후보 전체를 다시 검색하지 않고 두 지문만 비교하므로 활성 연결 전수점검에도 쓸 수 있다.
export function compareImageFingerprints(
  source: ImageFingerprint,
  target: ImageFingerprint,
) {
  const sourceVariants: VariantHashes[] = [source, ...(source.variantHashes ?? [])];
  const targetVariants: VariantHashes[] = [target, ...(target.variantHashes ?? [])];
  let hashDistance = 64;
  for (const left of sourceVariants) {
    for (const right of targetVariants) {
      hashDistance = Math.min(
        hashDistance,
        weightedHashDistance(left, right.phash, right.dhash, right.ahash),
      );
    }
  }
  const hashScore = hashScoreFromDistance(hashDistance);
  const hasFeatures = source.descriptors.length > 0 && target.descriptors.length > 0;
  const featureMatch = hasFeatures
    ? compareOrbDescriptors(source.descriptors, target.descriptors)
    : { orbMatchScore: 0, orbMatchCount: 0, homographyScore: 0, homographyInliers: 0 };
  const histogramScore = source.colorHistogram?.length && target.colorHistogram?.length
    ? histogramSimilarity(source.colorHistogram, target.colorHistogram)
    : null;
  const clipScore = source.clipEmbedding?.length === clipEmbeddingDim && target.clipEmbedding?.length === clipEmbeddingDim
    ? (cosineSimilarity(source.clipEmbedding, target.clipEmbedding) + 1) / 2
    : null;
  ...
  return { score, hashScore, histogramScore, ...featureMatch };
}
