...
async function resolveShopifyPriceString(product: Product) {
  ...
  return resolveListingPriceUsd(product, settings)?.priceUsd.toFixed(2);
}
...
async function syncShopifyGroupImages(product: Product, userId: string) {
  ...
  if (snapshot.errors?.length || !remote || remote.pageInfo.hasNextPage || remote.nodes.length !== products.length || products.length < 2 || new Set(remote.nodes.map((v) => v.sku)).size !== products.length || products.some((p) => !remote.nodes.some((v) => v.sku === p.sku && idFromGid(v.id) === p.shopifyVariantId))) {
    throw new ShopifyApiError("저장된 전체 옵션과 Shopify 옵션이 일치하지 않아 묶음 이미지 교체를 중단했습니다.", 409, null);
  }
 ...
 *
 * Existing Shopify identifiers are required. The price request runs alongside
 * location resolution, and inventory is sent immediately when the location is
 * known. With SHOPIFY_LOCATION_ID configured this completes in one network
 * round instead of the full upload path's three to four serial requests.
 */
export async function syncShopifyPriceAndInventory(
  ...
  const price = await resolveShopifyPriceString(product);
  if (!price) {
    await holdShopifyVariantForMissingPrice(input => shopifyApiRequest(config, input), {
  ...
  const inventoryPromise = Promise.all([locationIdPromise, pricePromise]).then(([locationId]) => {
    if (!locationId) {
      throw new ShopifyApiError(
        "Shopify의 활성 재고 위치를 찾을 수 없습니다.",
        409,
