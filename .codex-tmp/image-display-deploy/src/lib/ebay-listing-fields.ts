...
import { unitCardTitle, unitMembersDescription } from "@/lib/unit-card-label";
...
export function clampAspectValue(value: string, max = EBAY_ITEM_SPECIFIC_MAX_LENGTH) {
  if (value.length <= max) {
    return value;
  }
...
// Approved display abbreviations. Keep the original catalog/event names for
// descriptions and grouping; do not remove edition/country words generically.
const compactEventAlbums = new Map([
  ["bts|2017 bts live trilogy episode iii the wings tour in japan mini photo card ver.2", "2017 THE WINGS TOUR in Japan MINI PHOTO CARD VER.2"],
  ["stray kids|stray kids 2nd world tour maniac encore in japan saitama super arena", "MANIAC ENCORE in JAPAN SAITAMA SUPER ARENA"],
  ["stray kids|2nd world tour maniac encore in japan saitama super arena", "MANIAC ENCORE in JAPAN SAITAMA SUPER ARENA"],
]);

function compactEventTitle(brand: string, album: string, member = "") {
  const compact = compactEventAlbums.get(`${normalizedKey(brand)}|${normalizedKey(album)}`);
  if (!compact) return null;
  const title = uniqueTexts([brand, member, compact, "Photocard"]).join(" ");
  return title.length <= 80 ? title : null;
}

export function buildEbayListingTitle(product: EbayListingFieldProduct) {
  const unitTitle = unitCardTitle(product);
  ...
  const compact = compactEventTitle(brand, text(product.category), member);
  if (compact) return compact;
  const album = albumTitle(listingAlbumContext(brand, product.category));
...
// A variation parent represents selectable cards, not the first member's card.
// Keep the established keyword/alias/80-character rules in the same title builder.
export function buildEbayVariationListingTitle(group: {
  groupName: string;
  albumName: string;
  versionName: string;
  products: EbayListingFieldProduct[];
}) {
  const first = group.products[0];
  if (!first) throw new Error("옵션 상품이 없습니다.");
  const compact = compactEventTitle(group.groupName, group.albumName);
  if (compact) return compact;
...
export function buildEbayListingDescription(product: EbayListingFieldProduct) {
  const members = unitMembersDescription(product);
  const description = text(product.descriptionHtml);
  if (description) {
    return (product.descriptionHtml ?? description) + members;
  }

  const memo = text(product.memo);
  // Import audit records contain supplier URLs and internal account/file data.
  // They are never customer-facing copy, including truncated JSON records.
  const isImportRecord = /"(?:importedRow|legacyEbayIds)"\s*:/.test(memo);
  if (memo && !isImportRecord) {
    return members ? `<p>${escapeHtml(memo)}</p>${members}` : memo;
  }

  const title = buildEbayListingTitle(product);
  return `<p>${escapeHtml(title)}</p>${members}`;
}
...
export function buildEbayListingImageUrls(
  ...
  // This is the approved, channel-ready gallery also used by Shopify. The
  // user image can be an older raw capture retained for matching/editing.
