import { aiProductAllowedSql, aiJobAllowedSql, assertAiJobAllowed } from "@/lib/ai-image-policy";
...
export async function createAiJobs(limit: number) {
  ...
  return prisma.$executeRaw`
    ...
    WHERE ${aiProductAllowedSql} AND j."id" IS NULL AND p."image_url" IS NOT NULL AND p."image_url"<>''
...
export async function processNextAiJob() {
  ...
  >`
    ...
    WHERE "id"=(SELECT "id" FROM "ai_image_jobs" WHERE ${aiJobAllowedSql} AND "status"='queued' ORDER BY "created_at" FOR UPDATE SKIP LOCKED LIMIT 1)
...
async function claimAiImageApiBatchJob(batchId: string) {
  ...
  >`
    WITH candidate AS (
      ...
      WHERE ${aiJobAllowedSql} AND "status"='queued'
      ORDER BY "created_at"
