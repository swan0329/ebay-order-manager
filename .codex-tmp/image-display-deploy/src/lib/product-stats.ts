...
async function readProductStats(channel: ProductSalesChannel) {
  ...
  const [row] = await prisma.$queryRaw<ProductStats[]>`
    WITH stats_products AS MATERIALIZED (
      SELECT "brand", "stock_quantity", "user_front_image_url", "image_source",
    ...
    SELECT
      ...
      COUNT(*) FILTER (WHERE ${supply} AND NOT ${imageReady} AND ${aiProductAllowedSql})::int AS "aiImagePendingCount",
