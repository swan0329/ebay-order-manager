...
export function parsePocamarketProductState(
  ...
  if (config.responseMode === "CARD_SELL_LIST") {
    ...
    const price = Math.min(...parsed.map((row) => row.price));
    return {
      price,
      isSoldOut: false,
      availableCount: parsed.filter(row => row.price <= procurementPriceLimit(price)).reduce((sum, row) => sum + row.count, 0),
    };
  }
...
export async function fetchPocamarketProductState(
  ...
  const remainingMs = () => {
    const remaining = (dependencies.deadlineAt ?? Infinity) - Date.now();
    if (remaining <= 0) throw new Error("Pocamarket request timeout: time budget exhausted");
    return remaining;
  };
  const sleep = async (milliseconds: number) => {
    await rawSleep(Math.min(milliseconds, remainingMs()));
    remainingMs();
  };
  ...
  for (const [templateIndex, template] of templates.entries()) {
    ...
    for (let attempt = 0; attempt <= config.maxRetries; attempt += 1) {
      const timeoutMs = Math.max(1, Math.floor(Math.min(
        config.requestTimeoutMs ?? 10_000, remainingMs(),
      )));
      ...
      if (response.status === 401 || response.status === 403) {
        await sleep(backoffDelayMs(config.baseBackoffMs, attempt)).catch(() => {});
      ...
      if (response.status === 429) {
        ...
        try {
          ...
          throw new PocamarketBlockingError("포카마켓 요청 제한으로 수집을 중단합니다.", 429);
        }
        continue;
      }
