...
export async function getEbayFeedOperationTargets(
  ...
  const inventoryLogs = await prisma.syncLog.findMany({ where: { userId, type: "EBAY_INVENTORY_REFLECTION", status: "SUCCESS" }, orderBy: { createdAt: "desc" }, take: 10000, select: { rawJson: true } });
  const inventoryStates = new Map<string, { itemId: string; price?: string; quantity: number }>();
  for (const log of inventoryLogs) {
    const value = log.rawJson as { productId?: string; itemId: string; price?: string; quantity: number } | null;
    if (value?.productId && !inventoryStates.has(value.productId)) inventoryStates.set(value.productId, value);
  }
  const targets = products.flatMap((product) => {
    ...
    const inventoryStateChanged = !inventoryState || inventoryState.itemId !== (targetItemId ?? current?.itemId) ||
...
function publicJob(job: EbayFeedJob) {
  ...
  return {
    ...
    targets: targets.map(({ productId, sku, productName, itemId }) => ({ productId, sku, productName, itemId })),
    failures,
  };
}

export async function listEbayFeedJobs(userId: string) {
  const jobs = await prisma.ebayFeedJob.findMany({ where: { userId }, orderBy: { createdAt: "desc" }, take: 20 });
...
export async function submitEbayFeedOperation(userId: string, operation: EbayFeedOperation, limit?: number, retryJobId?: string, productIds?: string[]) {
  ...
  try {
    const account = await getActiveEbayInventoryAccount(userId);
    if (operation === "revise" && targets.some(target => target.quantity === 0)) {
    ...
    form.set("file", new Blob([buildEbayFeedXml(operation, targets)], { type: "application/xml" }), `${operation}-${job.id}.xml`);
...
async function applyCompletedJob(jobId: string, resultXml: string, summary: { successCount?: number; failureCount?: number }) {
  ...
  if (job.operation === "revise") {
    ...
    const pending = targets.filter(target => parsed.succeeded.has(target.productId) && !target.inventoryApplied && !target.inventoryError);
    const chunk = pending.slice(0, 25);
    const currentProducts = await prisma.product.findMany({ where: { id: { in: chunk.map(target => target.productId) } } });
    ...
    for (const target of chunk) {
      ...
      try {
        ...
        const changedWhileQueued = currentPrice !== target.price || currentQuantity !== target.quantity;
        target.price = currentPrice;
        target.quantity = currentQuantity;
        const reflected = await reflectEbayInventoryTarget(account, target);
        if (reflected.legacy && changedWhileQueued) throw new Error("대기 중 가격·수량이 변경되었습니다. 최신 값으로 다시 변동 처리해 주세요.");
    ...
    const remaining = targets.some(target => parsed.succeeded.has(target.productId) && !target.inventoryApplied && !target.inventoryError);
    if (remaining) {
...
export async function refreshEbayFeedJob(userId: string, jobId: string) {
  ...
  try {
    ...
    await prisma.ebayFeedJob.update({ where: { id: job.id }, data: { status: externalStatus, externalStatus: statusBody as unknown as Prisma.InputJsonValue, successCount: Number(statusBody.uploadSummary?.successCount ?? 0), failureCount: Number(statusBody.uploadSummary?.failureCount ?? 0) } });
