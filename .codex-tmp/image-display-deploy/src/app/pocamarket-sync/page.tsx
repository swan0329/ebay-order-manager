import { PocamarketCatalogPanel } from "@/components/PocamarketCatalogPanel";
...
export default async function PocamarketSyncPage() {
  ...
  return <div className="min-h-screen bg-zinc-50"><TopNav loginId={user.loginId}/><main className="mx-auto max-w-[1600px] px-4 py-7 sm:px-6"><h1 className="text-2xl font-bold">포카마켓 최신화</h1><p className="mb-6 mt-1 text-sm text-zinc-600">예약 수집과 수동 최신화 진행 상황을 확인합니다.</p><PocamarketCatalogPanel/><PocamarketSyncClient initialBatches={props} initialSettings={{ enabled: settings.enabled, scheduledHour: settings.scheduledHour, dailyBatchSize: settings.dailyBatchSize, speedProfile: settings.speedProfile, priorityStrategy: settings.priorityStrategy }} initialSummary={summary}/></main></div>;
}
