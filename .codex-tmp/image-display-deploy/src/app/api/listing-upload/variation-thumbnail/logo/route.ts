...
import { getListingImageSettings, getVariationThumbnailLogo, saveVariationThumbnailLogo } from "@/lib/variation-thumbnail-settings";

const schema=z.object({dataUrl:z.string().max(3_000_000),fileName:z.string().max(180).optional()});
export async function GET(){try{const user=await requireApiUser();return Response.json(await getListingImageSettings(user.id))}catch(error){if(error instanceof UnauthorizedError)return jsonError("Unauthorized",401);return jsonError(asErrorMessage(error),500)}}
