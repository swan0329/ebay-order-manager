...
export async function POST(request: Request) {
  try {
    ...
    if (input.action === "workerClaim") {
      ...
      >`WITH next_job AS (
          SELECT "id" FROM "ai_image_jobs" WHERE ${aiJobAllowedSql} AND "status"='queued'
    ...
    if (input.action === "reprocess") {
      const count =
        await prisma.$executeRaw`UPDATE "ai_image_jobs" SET "status"='queued',"preview_url"=NULL,"error"=NULL,"reviewed_at"=NULL,"reviewed_by"=NULL WHERE ${aiJobAllowedSql} AND "status" IN ('review','held','pass_ready','processing')`;
