...
export function PocamarketSyncClient({
  ...
  async function start() {
    ...
    if (!window.confirm(`선택 그룹: ${targetGroup || "전체"}. 해당 그룹에서 최대 ${actualCount.toLocaleString()}개를 최신화할까요? 다음 실행은 그다음 상품부터 이어집니다.`)) return;
    setBusy(true); setMessage("");
    const response = await fetch("/api/pocamarket-sync/batches", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ limit: requestedCount, group: targetGroup || undefined }),
    });
    const body = await response.json();
  ...
  async function startUnsynced() {
    if (
      !window.confirm(
        `선택 그룹: ${targetGroup || "전체"}. 아직 정상 반영되지 않은 상품과 미시도 상품을 최신화할까요?`,
      )
    )
      return;
    setBusy(true); setMessage("");
    const response = await fetch("/api/pocamarket-sync/batches", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ scope: "unsynced", group: targetGroup || undefined }),
    });
    const body = await response.json();
    ...
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-end gap-2">
          <label className="text-xs font-semibold text-zinc-500">대상 그룹
            <select aria-label="최신화 대상 그룹" value={targetGroup} onChange={(event) => setTargetGroup(event.target.value)} className="ml-2 rounded-xl border border-zinc-300 px-3 py-2 text-sm text-zinc-900">
              <option value="">전체 그룹</option><option value="BTS">BTS만</option><option value="Stray Kids">스트레이키즈만</option>
            </select>
          </label>
